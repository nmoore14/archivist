...
System prompts live in `internal/rag/prompts/`, with one Markdown file per prompt. The files are embedded in the application binary, so prompt edits take effect after rebuilding or restarting the development build. Request-specific context and student questions remain separate user messages.
